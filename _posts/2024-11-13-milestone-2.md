...
We did a few internet research and came with a default neural network configuration to start with:  
- Input layer has x inputs, where x is the number of features.  
- 1 Hidden layer of 64x32 neurons, with ReLU activation.  
...
- Training loop with a mini-batch of 64 data points, and use training set over 10 epcohs


Then, we were interested to compare the performance of the model by tuning the following feature selection / hyper parameters:

### 6.1 Feature selection  

The tidy dataset contains 10 input features (See section 4.5). We wanted to compare the performances of different selection of the 10 input feautres, from a small number to all of them.
For the categorical features (shot type and lastEventType), we have "one-hot-encoded" them before the training.

The different experiments were tracked in W&B, runs x to here:  
[Neural Network Feature Selection experiement in W&B](https://wandb.ai/IFT6758-2024-B04/ms2-q6-best-shot-V4?nw=nwusersergemalo)
...
In the graphs above, we can see that only using only one feature, the distance, seems to give better results. However, we are still interested in knowing if using more than just this single feature will give better results when we will change other parameters.
...
[Neural Network Mini-batch size experiement in W&B](https://wandb.ai/IFT6758-2024-B04/ms2-q6-best-shot-BatchSize?nw=nwusersergemalo)
