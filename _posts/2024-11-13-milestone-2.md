...
We were interested to compare the performance of the following hyper.

1. Feature selection
2. Neural Network architecture (Number of hidden layers, number of neurons)
3. Training with diffent mini-batch sizes
4. 

The strategy we used was
1. Build a simple basic neural nework
2. For each hyperparameter listed above
2.1 Train the model
2.2 Validate the model (measure performance and log it to W&B)
2.3 Vary the hyper-parameter and restart at step 2.1
3. Select the model with the best performance and repeat loop for the next hyper-parameter

At each train/validation iteration, we log the results into W&B. 
Config  
Epochs  10   
Loss over Epochs
ROC AUC over Epochs
Summary: 4 graphs and ROC AUC  


### 6.1.1 Feature selection
### 6.1.2 Neural Network architecture
### 6.1.3 Mini-batch sizes
