...
### 6.4 Learning rate  
Trainig rate can have a major impact on the performance of the model. When it is too low, we might not optimize until the minimal gradient. When too high, we "jump" over the minimal gradient and never find it.  
We tested the following training rates, which are "around" the default value of 0.001:  
...
- Loss function Binary Cross-Entropy Loss (BCELoss in PyTorch)  
- Adam optimizer with a learning rate of xxx  
- Perform training with xxx epochs  
